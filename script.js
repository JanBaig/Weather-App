...
//  Get the user's location using the  W3C Geolocation API
...
function getWeather(longitude, latitude){
    // Always add the https://
    let api = `https://api.openweathermap.org/data/2.5/weather?lat=${latitude}&lon=${longitude}&appid=${api_key}`

    // Returns a promise
    fetch(api)

    // When response successfully arives we can work with it
    .then(function(response){
        let data = response.json()
        //console.log(data)
        return data //promise
    })
    // when data is resolved! Since it's a promise
    .then(function(data){
        //Object with all needed properties
        //console.log(data) 
        let data_object = {
...
function displayMap(longitude, latitude){
    ...
    btn1Element.addEventListener("click", function(){ 
        ...
        document.getElementById('legend_title').innerHTML = 'Temp (°C)'
    ...
    btn2Element.addEventListener("click", function(){ 
        ...
        document.getElementById('legend_title').innerHTML = 'Wind Speed (m/s)'
        document.getElementById('legend').style.display = "block";
    ...
    btn3Element.addEventListener("click", function(){ 
        ...
        document.getElementById('legend_title').innerHTML = 'Rain (mm)'
        document.getElementById('legend').style.display = "block";
    ...
    btn4Element.addEventListener("click", function(){ 
        ...
        document.getElementById('legend_title').innerHTML = 'Pressure (Pa)'
        document.getElementById('legend').style.display = "block";
    ...
    btn5Element.addEventListener("click", function(){ 
        ...
        document.getElementById('legend_title').innerHTML = 'Clouds (0-100%)'
        document.getElementById('legend').style.display = "block";
    ...
    // Popup Display
    pin.bindPopup(`You're Here!`).openPopup();
